...
- **Slice 1 so far**
  ...
  - **Testing** — 73 tests / 18 projects. Unit: uow atomicity + audit query/pagination, service audit/outbox wiring, applicability evaluator, pack validation, readiness derivation, capability matrix. Integration (Postgres, `TEST_DATABASE_URL`, else skipped): RLS hides cross-tenant rows entirely, `WITH CHECK` rejects cross-tenant inserts, a failed uow rolls back the entity **and** its audit event, `audit_event` `UPDATE`/`DELETE` denied to `rre_app` (append-only), `queryAudit` is tenant-scoped and newest-first, `pack_key` isolation. **UAT** (`apps/api/src/acceptance/`): `AC-003` (scope + applicability recorded, no silent pack fallback), `AC-004` (counts reconcile, neutral states not counted, no "compliant"/"certified" language), `AC-018` (audit reconstruction — who/what/when recoverable, no tokens or secrets in the trail).

Next: the outbox relay worker (`@rre/worker` → LocalStack SQS), then the operator web shell and the `ENT-001` / `MAT-001` screens against these endpoints.
