...
Full specification, a buildable monorepo, and the first slices of real engine code. `typecheck` / `lint` / `lint:copy` / `test` (134) / `build` / `format` all green.
...
- **Slice 1 so far**
  ...
  - **Testing** — 134 tests / 32 projects. Unit: readiness derivation (every precedence branch + entity-status roll-up), claim assert/approve/reject/supersede + reason enforcement, uow atomicity + audit pagination, applicability evaluator, pack validation, capability matrix, SQS command shape, request-loop token hashing + not-applicable-control rejection + expired-grant handling, draft round-trip + overwrite + clear-on-submit + outbox notification, resend (old link dies / new link works / reactivates a lapsed request / refuses a closed one); web screens (form from schema, matrix banner + readiness column + filter + inline claim + refetch, review-queue approve/reject with reason + server-error surfacing, request builder offering only applicable controls + one-time link reveal, request detail accept-into-review + revoke + reissue, contributor portal least-disclosure + no-account submit + generic invalid-link message + draft rehydrate + save-progress). Integration (Postgres, `TEST_DATABASE_URL`, else skipped): RLS hides cross-tenant rows (entities, audit, claims, review decisions, requests, submissions, drafts) entirely; `WITH CHECK` rejects cross-tenant inserts; grants resolve by hash with no tenant context; `request_draft` is RLS-scoped **and** mutable (upsert overwrites, delete clears); the expiry sweep flips only lapsed non-terminal requests and writes their audit + outbox trail, cross-tenant, idempotently; a failed uow rolls back the entity **and** its audit event; `audit_event` / `review_decision` / `contributor_submission` `UPDATE`/`DELETE` denied to `rre_app`; `claim.value` is immutable to the app; the relay and the expiry sweep each run in their own disposable database. **UAT** (`apps/api/src/acceptance/`): `AC-003`, `AC-004`, `AC-006`/`AC-007` (least-disclosure contributor link + no-account submission), `AC-010`, `AC-018`.

Next: request expiry sweep (a scheduled job moving lapsed requests to `EXPIRED`) and an operator "resend / new link" action.
