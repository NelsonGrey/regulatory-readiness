...
- **Slice 1 so far**
  ...
  - `@rre/api` — `GET /api/v1/packs` and **`GET /api/v1/packs/:packKey`** (entity-facts schema + control families, drives the ENT-001 form); **`POST /api/v1/entities`** creates a regulated entity with an immutable, hashed scope evaluation; **`GET /api/v1/entities/:id/matrix`** returns the per-control applicability matrix + honest state counts; **`GET /api/v1/audit-events`** (AUD-001) — newest-first, filter by target/action/since, cursor pagination. Tenant comes from `x-tenant-id` (dev stand-in for OIDC + RLS); cross-tenant reads are empty/404.
  - **Persistence** — migrations `0001` (entities + scope evaluations, RLS `FORCE`d), `0002` (`rre_app` runtime role), `0003` (append-only `audit_event` + transactional `outbox`), `0004` (`rre_relay` BYPASSRLS role, column-limited to `outbox` bookkeeping); forward-only SQL runner (`pnpm db:migrate`).
  ...
  - **Testing** — 83 tests / 22 projects. Unit: uow atomicity + audit query/pagination, service wiring, applicability evaluator, pack validation, readiness derivation, capability matrix, SQS command shape, web screens (form generated from schema, POST body shape, 422 field errors, matrix rendering + filter + not-found). Integration (Postgres, `TEST_DATABASE_URL`, else skipped): RLS hides cross-tenant rows entirely, `WITH CHECK` rejects cross-tenant inserts, a failed uow rolls back the entity **and** its audit event, `audit_event` `UPDATE`/`DELETE` denied to `rre_app`, `queryAudit` tenant-scoped, `pack_key` isolation; the relay runs in its own disposable database — drains across tenants (BYPASSRLS), retries on failure, cannot rewrite `topic`/`payload`. **UAT** (`apps/api/src/acceptance/`): `AC-003`, `AC-004`, `AC-018`.

Next: contributor request loop (`REQ-*` / `SUP-*`) — scoped no-account links, or the claim/review model (`REV-*`), depending on priority.
