...
The engine should make a complicated evidence project feel controlled without making it look deceptively complete. At any moment, an authorized user should be able to answer:

- What regulated entity and control snapshot am I looking at?
- Which controls are required, conditional, not yet required, or not applicable?
...
| --- | --- | --- | --- |
| `ORG_ADMIN` | Workspace identity, MFA required | `DASH-001` | Entire tenant and administration |
| `COMPLIANCE_MANAGER` | Workspace identity | `DASH-001` | Entire assigned tenant workflow |
| `TECHNICAL_APPROVER` | Workspace identity | `REV-001` | Assigned entities/reviews; no user/billing administration |
...
`SUPPLIER_CONTRIBUTOR` is the **external contributor** principal — any outside party (supplier, vendor, agency, processor, partner). The UI MUST never use a role to imply legal authority: "Technical Approver" means authorized inside the customer workspace, not approved by an authority.

## 3. Capability model

| Capability | Admin | Manager | Approver | Reviewer | Contributor |
| --- | ---: | ---: | ---: | ---: | ---: |
| View portfolio/entities | Yes | Yes | Assigned/all by policy | Shared only | Request context only |
| Edit entity facts | Yes | Yes | Proposed correction only | No | No |
...
Organization
  └─ (optional Product grouping)
      └─ Regulated entity  ──  bound to exactly one control pack
          ├─ Selected control snapshot
          ├─ Control rows
          │    ├─ applicability result
          │    ├─ owner/party assignment
...
Users should not have to understand the database. Navigation and copy consistently use "regulated entity," "control," "request," "document," "proposed value," "approved claim," "evidence," "snapshot," and "export."
...
- `/w/:workspaceId/entities/new` — entity creation (`ENT-001`)
- `/w/:workspaceId/entities/:entityId` — entity overview (`ENT-002`)
- `/w/:workspaceId/entities/:entityId/matrix` — control matrix (`MAT-001`)
- `/w/:workspaceId/entities/:entityId/controls/:controlKey` — control detail (`MAT-002`)
- `/w/:workspaceId/entities/:entityId/parties` — external-party ownership (`SUPP-001`)
- `/w/:workspaceId/entities/:entityId/requests` — request list (`REQ-001`)
- `/w/:workspaceId/entities/:entityId/requests/new` — request builder (`REQ-002`)
...
- `/w/:workspaceId/entities/:entityId/snapshots` — snapshot history (`SNP-001`)
- `/w/:workspaceId/entities/:entityId/snapshots/new` — snapshot preflight (`SNP-002`)
- `/w/:workspaceId/entities/:entityId/exports` — exports/shares (`EXP-001`)
- `/w/:workspaceId/audit` — audit history (`AUD-001`)
- `/w/:workspaceId/settings/*` — organization, members, security, retention, integrations, plan (`SET-001`–`SET-006`)

### 5.3 External portals

- `/c/r/:requestToken` — contributor request (`SUP-001` through `SUP-006`)
...
- Entity pages add secondary tab navigation: Overview, Matrix, Parties, Requests, Snapshots, Exports.
- Breadcrumbs show workspace > (product) > entity > current object.

### 6.2 Tablet browser

- Collapsible navigation rail, default collapsed below 1024 px.
- Entity tab row scrolls horizontally with visible end fade and keyboard support.
- Split panes become 40/60 or stacked depending on width and content.

### 6.3 Phone browser

- Top app bar with workspace/entity title and menu button.
- Bottom navigation: Portfolio, Review, Documents, More.
- Entity subnavigation appears as an accessible select/sheet, not a crushed tab row.
...
Required manual coverage: iPhone-sized Safari (portrait/landscape where forms support it); iPad-sized Safari (portrait/landscape); Android phone Chrome; Android tablet Chrome; desktop Chrome/Edge, Firefox, and Safari. The MVP is responsive web, not a claim of native-app support.
...
No all-green "compliant" banner is permitted. `EVIDENCE_READY` appears as "Evidence ready for snapshot &lt;snapshot key&gt;" followed by the limitation: "This is a preparation status, not certification or authority approval."
...
| `EntityContextBar` | Product/entity/classification/target date/snapshot always visible on entity work |
| `ControlRow` | Control identifier/name, applicability, state, owner, source, last change, action |
...
| `AccessClassBadge` | Public candidate/restricted/authority/internal/party confidential |
...
- Contributor-provided content is shown as untrusted assertion until reviewed.
...
Every applicable screen MUST define: loading skeleton or progress; first-use empty state with a single next action; filtered empty state with clear reset; inline validation error; permission denied without revealing target existence; offline/interrupted state with retained draft when safe; session expiry and re-authentication; asynchronous job queued/running/failed/succeeded; source snapshot outdated; deleted/archived object; feature unavailable due to `GATE`.

## 12. Accessibility contract

`MVP/GATE` until independently verified.

- WCAG 2.2 AA design intent; full keyboard operation and visible focus.
...
- Timeouts warn and allow extension; contributor drafts survive ordinary expiry where contractually allowed.
...
- The active pack's authority disclaimer is reachable from each matrix and snapshot.
...
- Product analytics must not capture document text, claim values, contributor responses, or filenames containing sensitive information.
...
- Request language is selected per request/recipient; unsupported languages fall back visibly to English.

## 15. Cross-platform acceptance baseline

1. All primary operator workflows are usable on tablet and desktop.
2. Contributor request completion is fully usable at 320 px width.
