...
- Select and review the scheduler and cookieless analytics provider.
...
Variables: `PUBLIC_SITE_URL`, `PUBLIC_APP_URL`, `PUBLIC_SCHEDULER_URL`, optional `PUBLIC_ANALYTICS_DOMAIN`, optional `PUBLIC_ANALYTICS_SRC`, `MARKETING_BUCKET`, and `CLOUDFRONT_DIST_ID`.
...
2. Verify canonical URLs, scheduler behavior, zero cookies on initial load, metadata, and the manual accessibility checklist on the staging domain.
